 ...
 * Soft, muted color scheme - easy on the eyes
 */
...
export default function CompanySettings() {
  ...
  return (
    <div className="p-3 sm:p-6 space-y-4 sm:space-y-6">
      ...
      {/* Main Container - Softer colors */}
      <div className="bg-slate-100 rounded-xl border border-slate-200 overflow-hidden shadow-sm">
        {/* Tabs - Muted background */}
        <div className="border-b border-slate-200 bg-slate-200/60">
          <div className="flex overflow-x-auto">
            {tabs.map(tab => (
              ...
              >
                ...
                <span className="bg-slate-300 text-slate-600 text-xs px-2 py-0.5 rounded-full">{tab.count}</span>
              </button>
            ))}
          </div>
        </div>
          ...
          {loading && companies.length === 0 ? (
            ...
            <>
              ...
              {activeTab === 'companies' && (
                <div className="space-y-4">
                  <div className="flex justify-between items-center">
                    <h2 className="text-lg font-semibold text-slate-800">Companies ({companies.length})</h2>
                    <button
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {companies.map(company => (
                      <div key={company.id} className="bg-slate-50 border border-slate-200 rounded-xl p-4 hover:bg-slate-100 transition-colors">
                        <div className="flex items-start justify-between mb-3">
                          <div className="flex items-center gap-3">
                            <div className="w-12 h-12 rounded-xl bg-slate-200 flex items-center justify-center text-xl">🏢</div>
                            <div>
                              <h3 className="font-semibold text-slate-800">{company.name}</h3>
                              <p className="text-sm text-slate-500 font-medium">{company.code || '—'}</p>
                            </div>
                          </div>
                        </div>
                        {company.address && (
                          <p className="text-sm text-slate-500 mb-3 line-clamp-2">📍 {company.address}</p>
                        )}
                        <div className="flex items-center justify-between pt-3 border-t border-slate-200">
                          ...
                          <div className="flex gap-2">
                            <button
                              ...
                              className="px-3 py-1.5 text-sm text-slate-600 hover:bg-slate-200 rounded-lg font-medium transition-colors"
                            >
                              Edit
                            </button>
                            <button
                    ...
                    {companies.length === 0 && (
                      <div className="col-span-full text-center py-12">
                        ...
                        <button onClick={() => openCompanyModal()} className="mt-3 text-slate-600 font-medium hover:text-slate-800">
                ...
                <div className="space-y-4">
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                    <div className="flex items-center gap-3">
                      ...
                      <select
                        ...
                        className="px-3 py-2 bg-slate-50 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-slate-400 focus:border-slate-400"
                      >
                        {companies.map(c => (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {departments.map(dept => (
                      <div key={dept.id} className="bg-slate-50 border border-slate-200 rounded-xl p-4 hover:bg-slate-100 transition-colors">
                        <div className="flex items-start justify-between">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-sm text-slate-500">{dept.code || 'No code'}</p>
                            </div>
                          </div>
                    ...
                    {departments.length === 0 && (
                      <div className="col-span-full text-center py-12">
                        ...
                        <button onClick={() => openDepartmentModal()} className="mt-3 text-slate-600 font-medium hover:text-slate-800">
              ...
              {activeTab === 'cafeterias' && (
                <div className="space-y-4">
                  <div className="flex justify-between items-center">
                    <h2 className="text-lg font-semibold text-slate-800">Cafeterias ({cafeterias.length})</h2>
                    <button
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {cafeterias.map(cafeteria => (
                      <div key={cafeteria.id} className="bg-slate-50 border border-slate-200 rounded-xl p-4 hover:bg-slate-100 transition-colors">
                        <div className="flex items-start justify-between mb-3">
                          <div className="flex items-center gap-3">
                            <div className="w-12 h-12 rounded-xl bg-slate-200 flex items-center justify-center text-xl">🍽️</div>
                            <div>
                              <h3 className="font-semibold text-slate-800">{cafeteria.name}</h3>
                              {cafeteria.location && <p className="text-sm text-slate-500">{cafeteria.location}</p>}
                            </div>
                          </div>
                        </div>
                        <div className="bg-slate-200/50 rounded-lg p-3 space-y-1 text-sm">
                          <div className="flex justify-between">
                            <span className="text-slate-500">Breakfast cutoff:</span>
                            <span className="font-medium text-slate-700">
                              {cafeteria.default_breakfast_cutoff || cafeteria.defaultBreakfastCutoff || '08:00'}
                            </span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-slate-500">Lunch cutoff:</span>
                            <span className="font-medium text-slate-700">
                    ...
                    {cafeterias.length === 0 && (
                      <div className="col-span-full text-center py-12">
                        ...
                        <p className="text-slate-500">No cafeterias found</p>
                        <button onClick={() => openCafeteriaModal()} className="mt-3 text-slate-600 font-medium hover:text-slate-800">
      ...
      {/* Company Modal - Softer colors */}
      {showCompanyModal && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-slate-50 rounded-xl w-full max-w-lg shadow-xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSaveCompany} className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2 sm:col-span-1">
                  ...
                  <input
                    ...
                    className="w-full px-4 py-2.5 bg-white border border-slate-300 rounded-lg focus:ring-2 focus:ring-slate-400 focus:border-slate-400"
              ...
              <div className="flex gap-3 pt-4 border-t border-slate-200">
                ...
                <button
                  type="submit"
                  className="flex-1 px-4 py-2.5 bg-slate-700 text-white rounded-lg hover:bg-slate-800 font-medium transition-colors"
                >
      ...
      {showDepartmentModal && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-slate-50 rounded-xl w-full max-w-md shadow-xl">
            <div className="p-6 border-b border-slate-200 bg-slate-100">
              <h2 className="text-xl font-bold text-slate-800">
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-slate-50 rounded-xl w-full max-w-md shadow-xl">
            ...
            <form onSubmit={handleSaveCafeteria} className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Cafeteria Name *</label>
                <input
                  type="text"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">Breakfast Cutoff</label>
                  <input
                    type="time"
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">Lunch Cutoff</label>
                  <input
                    type="time"
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-slate-50 rounded-xl w-full max-w-md shadow-xl">
            <div className="p-6 border-b border-slate-200 bg-red-50">
              <h2 className="text-xl font-bold text-red-700">🗑️ Delete {deleteTarget.type}</h2>
            </div>
            <div className="p-6">
              <div className="bg-slate-100 border border-slate-200 rounded-xl p-4 mb-4">
                <p className="font-medium text-slate-800">{deleteTarget.item.name}</p>
                {deleteTarget.item.code && <p className="text-sm text-slate-500">{deleteTarget.item.code}</p>}
              </div>
